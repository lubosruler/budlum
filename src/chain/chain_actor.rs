...
impl ChainHandle {
    ...
    pub async fn bond_relayer(
        ...
        let (tx, rx) = oneshot::channel();
        let _ = self.tx.send(ChainCommand::BondRelayer(address, amount, tx)).await;
        rx.await
            .unwrap_or_else(|_| Err("Actor dropped".to_string()))
    }
    ...
    pub async fn bond_prover(
        ...
        let (tx, rx) = oneshot::channel();
        let _ = self.tx.send(ChainCommand::BondProver(address, amount, tx)).await;
        rx.await
            .unwrap_or_else(|_| Err("Actor dropped".to_string()))
    }
    ...
    pub async fn submit_zk_proof(
        ...
        let (tx, rx) = oneshot::channel();
        let _ = self.tx.send(ChainCommand::SubmitZkProof(submission, tx)).await;
        rx.await
            .unwrap_or_else(|_| Err("Actor dropped".to_string()))
    }
        ...
        while let Some(cmd) = self.rx.recv().await {
            match cmd {
                ...
                ChainCommand::GetRegistryMember(account, role, res_tx) => {
                    let _ = res_tx.send(self.blockchain.get_registry_member(&account, role));
                }
                ChainCommand::GetRegistryActiveMembers(role, res_tx) => {
                    let _ = res_tx.send(self.blockchain.registry_active_members(role));
                }
                ...
                ChainCommand::BondRelayer(address, amount, res_tx) => {
                    let _ = res_tx.send(self.blockchain.state.bond_relayer(&address, amount));
                }
                ChainCommand::BondProver(address, amount, res_tx) => {
                    let _ = res_tx.send(self.blockchain.state.bond_prover(&address, amount));
                }
                ChainCommand::SubmitZkProof(submission, res_tx) => {
                    let _ = res_tx.send(self.blockchain.submit_zk_proof(submission));
                }
