...
pub struct Blockchain {
    ...
    /// Accepted ZK proof claims ("first valid wins" per (domain, height)).
...
impl Blockchain {
    ...
    pub fn submit_slashing_evidence(&mut self, evidence: SlashingEvidence) -> Result<(), String> {
        ...
        // Bridge consensus evidence into the permissionless registry using the
        // canonical report format. The evidence was just cryptographically
        // verified above, so it is marked ConsensusVerified and is actionable.
        if let Some(report) = Self::registry_report_from_evidence(&evidence) {
    ...
    /// Convert a verified consensus [`SlashingEvidence`] into the canonical,
    /// domain-agnostic [`SlashingReport`](crate::registry::SlashingReport).
    fn registry_report_from_evidence(
        evidence: &SlashingEvidence,
    ) -> Option<crate::registry::SlashingReport> {
        let offender = evidence.header1.producer?;
        Some(crate::registry::SlashingReport::consensus_double_sign(
            offender,
            evidence.header1.index,
            evidence.header1.hash.clone(),
            evidence.header2.hash.clone(),
    ...
    /// Submit a canonical [`SlashingReport`](crate::registry::SlashingReport)
    /// to the permissionless registry.
    ///
    /// This is permissionless: **anyone** may submit. There is no whitelist and
    /// no approval. Safety comes from the report itself — the registry only acts
    /// on reports that are structurally valid AND consensus-verified
    /// (`report.is_actionable()`), so unverified submissions are recorded/echoed
    /// but never cause an unproven slash.
    pub fn submit_registry_slashing_report(
        ...
        // Structural validation always runs and is domain-agnostic.
        report.validate_shape().map_err(|e| e.to_string())?;

        // Anti-spam fee: charged only for reports that carry a reporter (i.e.
        // externally-submitted ones). Consensus-internal reports (double-sign,
        // liveness) have `reporter: None` and pay nothing.
        let fee = self.state.registry.params().slashing_report_fee;
        let charged_reporter = match report.reporter {
            Some(reporter) if fee > 0 => {
                let balance = self.state.get_balance(&reporter);
                if balance < fee {
                    return Err(format!(
                        "insufficient balance for slashing report fee: have {balance}, need {fee}"
                    ));
                }
        ...
        // Applies only if actionable (consensus-verified); otherwise returns
        // an error the caller can surface without any state change.
        let result = self.state.registry.slash_from_report(&report);

        // Fee accounting: refund an actionable (correct) report so honest
        // accusers are not penalised; otherwise the fee is burned (anti-spam).
        if let Some(reporter) = charged_reporter {
            match &result {
                Ok(_) => {
                    // Actionable (Ok(Some)=slashed, Ok(None)=verified but
                    // offender unregistered): refund the fee.
                    self.state.add_balance(&reporter, fee);
                }
                Err(_) => {
                    // Unverified / not actionable: fee stays burned.
                }
            }
        }
    ...
    /// Record consensus participation for a completed epoch and apply any
    /// resulting liveness slashes through the existing report→slash flow.
    ///
    /// `participants` is the set of validators that participated this epoch.
    /// Returns the number of validators slashed for liveness this epoch. This
    /// does NOT open a new slashing path — each generated report is routed
    /// through `submit_registry_slashing_report` exactly like any other
    /// consensus-verified report.
    pub fn record_liveness_epoch(
        &mut self,
        epoch: u64,
        participants: &std::collections::HashSet<crate::core::address::Address>,
    ) -> usize {
        let reports = self.state.record_liveness_epoch(epoch, participants);
        let mut slashed = 0usize;
        for report in reports {
            match self.submit_registry_slashing_report(report) {
                Ok(Some(_)) => slashed += 1,
                Ok(None) => {}
                Err(e) => tracing::warn!("Liveness report rejected: {}", e),
            }
        }
        slashed
    }

    /// OBSERVE-MODE liveness (Tur 10, Decision 2.3): record consensus
    /// participation for a completed epoch and LOG any validators that crossed
    /// the miss threshold — WITHOUT applying any slash. The generated
    /// `SlashingReport`s are deliberately NOT routed to
    /// `submit_registry_slashing_report`, so no stake is cut and the registry is
    /// unchanged. Real slashing is a future turn's decision, made after this
    /// signal is validated on live/testnet data.
    ///
    /// Returns the number of validators that would have been reported (for
    /// metrics/tests), NOT the number slashed (which is always zero here).
    pub fn observe_liveness_epoch(
        ...
        for report in &reports {
            tracing::warn!(
                "LIVENESS (observe-only, no slash): validator {} crossed miss threshold at epoch {} (condition={:?})",
                report.offender,
                epoch,
                report.condition()
            );
        }
        // IMPORTANT: reports are intentionally dropped here — NOT sent to
        // submit_registry_slashing_report. No slash, no registry mutation.
        reports.len()
    }

    /// Derive the set of validators that participated in `epoch` from already
    /// available data (Decision 2.2: derive, do not store a new state field).
    ///
    /// Signal (Decision 2.1 = B, domain-specific union):
    ///  - Block producers of every block in the epoch's height range
    ///    (universal — PoW/PoS/BFT all set `block.producer` in `produce_block`).
    ///  - BFT quorum signers: for each finality certificate recorded for the
    ///    epoch, map its bitmap indices back to validator addresses via the
    ///    epoch's validator snapshot (`cert.signer_indices` → `snapshot.validators[i].address`).
    fn collect_epoch_participants(
        &self,
        epoch: u64,
    ) -> std::collections::HashSet<crate::core::address::Address> {
        use std::collections::HashSet;
        let mut participants: HashSet<crate::core::address::Address> = HashSet::new();

        // (1) Block producers in this epoch's height range. Epoch `e` covers
        // block heights (e * EPOCH_LENGTH, (e+1) * EPOCH_LENGTH]. Height 0 is
        // genesis (no producer).
        let start = epoch.saturating_mul(EPOCH_LENGTH).saturating_add(1);
        let end = epoch.saturating_add(1).saturating_mul(EPOCH_LENGTH);
        for height in start..=end {
            if let Some(block) = self.chain.get(height as usize) {
                if let Some(producer) = block.producer {
                    participants.insert(producer);
                }
            }
        }

        // (2) BFT quorum signers from finality certificates recorded for this
        // epoch, mapped bitmap-index -> address via the epoch validator snapshot.
        if let Some(certs) = self.pending_finality_certs.get(&epoch) {
            let snapshot = self.validator_snapshot_for_epoch(epoch);
            let n = snapshot.validators.len();
            for cert in certs {
                for idx in cert.signer_indices(n) {
                    if let Some(entry) = snapshot.validators.get(idx) {
                        participants.insert(entry.address);
                    }
                }
            }
        }
    ...
    /// Hook called after a block is committed & pushed to `self.chain`. When the
    /// block closes an epoch, derive that epoch's participants and run
    /// observe-mode liveness. No new state is stored (Decision 2.2).
    fn maybe_observe_liveness_on_epoch_close(&mut self, committed_index: u64) {
        if committed_index == 0 || !committed_index.is_multiple_of(EPOCH_LENGTH) {
            return;
        }
        // Block at a multiple of EPOCH_LENGTH closes epoch `e = index/EPOCH - 1`.
        let closed_epoch = (committed_index / EPOCH_LENGTH).saturating_sub(1);
        let participants = self.collect_epoch_participants(closed_epoch);

        // Tur 12: dispatch on the governance/config flag.
        // - enabled  -> record_liveness_epoch: REAL slash via the existing
        //   report->slash flow (built & tested in Tur 3, now reachable from live
        //   block production).
        // - disabled -> Tur 10 observe-only (report + log, no economic effect).
        // Default is disabled (see RegistryParams::liveness_slashing_enabled).
        if self.state.registry.params().liveness_slashing_enabled {
            let slashed = self.record_liveness_epoch(closed_epoch, &participants);
            if slashed > 0 {
                tracing::warn!(
                    "Liveness slash: epoch {} closed, {} validator(s) slashed for downtime",
                    closed_epoch,
                    slashed
                );
            }
        } else {
            let reported = self.observe_liveness_epoch(closed_epoch, &participants);
            if reported > 0 {
                tracing::info!(
                    "Liveness observe (slashing disabled): epoch {} closed, {} validator(s) over threshold (no slash)",
                    closed_epoch,
                    reported
                );
            }
    ...
    /// Read a permissionless registration (validator/verifier/relayer/…).
    pub fn get_registry_member(
        &self,
        account: &crate::core::address::Address,
        role: crate::registry::RoleId,
    ) -> Option<crate::registry::Registration> {
        self.state.registry.get(account, role).cloned()
    }

    /// List active members of a role in the permissionless registry.
    pub fn registry_active_members(
        &self,
        role: crate::registry::RoleId,
    ...
    pub fn verify_domain_commitment_finality(
        ...
        let status = match domain.kind {
            ...
            ConsensusKind::Zk => {
                ...
                self.ensure_adapter_name(domain, adapter.adapter_name())?;
                // Tur 5 (Option B): resolve ZK finality against the single
                // source of truth — the ProofClaimRegistry, which only holds
                // proofs that already passed real STARK verification in
                // `submit_zk_proof`. Look up the accepted claim for this
                // (domain, height) and let the adapter enforce the bindings.
                let accepted_claim_root = if let crate::domain::FinalityProof::Zk {
                    domain_id,
                    target_height,
                    ..
                } = proof
                {
                    self.proof_claims
                        .get(&crate::prover::ProofClaimKey {
                            domain_id: *domain_id,
                            target_height: *target_height,
                        })
                        .map(|claim| claim.final_state_root)
                } else {
                    None
                };
                adapter.verify_finality_with_claim(
                    domain,
                    commitment,
                    proof,
                    accepted_claim_root,
    ...
    /// Internal, unauthenticated primitive: insert & persist a cross-domain
    /// message. Used by system-generated bridge event paths (lock/burn) where
    /// the message originates from already-authorized on-chain logic.
    ///
    /// For messages originating from an external relayer (RPC / p2p gossip),
    /// use [`Self::submit_relayed_cross_domain_message`] which enforces that the
    /// sender is an active relayer.
    ...
    /// Canonical `MessageKind` payload marker for ZK proof submissions.
    pub const ZK_PROOF_MESSAGE_KIND: &'static [u8] = b"zk-proof";

    /// Permissionless ZK proof submission (the L1 ↔ BudZKVM bridge).
    ///
    /// Fully open: anyone may submit; registration is not required for a valid
    /// proof to be accepted (STARK proofs are self-verifying). The `PROVER` role
    /// only affects reward eligibility.
    ///
    /// Flow:
    /// 1. Structural checks: message kind + payload-hash binding.
    /// 2. Anti-spam fee charged to the submitter (refunded if the proof
    ///    verifies, burned otherwise).
    /// 3. Core-native STARK verification via `bud_proof`.
    /// 4. "First valid wins" claim policy per (domain, height): new claim
    ///    accepted; identical claim is idempotent; conflicting final state root
    ///    is rejected.
    /// 5. Reward paid iff the submitter is a registered active prover.
    ///
    /// Returns the acceptance outcome.
    pub fn submit_zk_proof(
        &mut self,
        submission: crate::prover::ZkProofSubmission,
    ) -> Result<crate::prover::ProofAcceptance, String> {
        use crate::prover::{ClaimDecision, ProofAcceptance, ProofClaimKey, ProofError};

        // 1a. Message kind must be the ZK-proof kind.
        let expected_kind =
            crate::cross_domain::MessageKind::Custom(Self::ZK_PROOF_MESSAGE_KIND.to_vec());
        if submission.message.kind != expected_kind {
            return Err(ProofError::WrongMessageKind.to_string());
        }
        // 1b. The message payload hash must bind to the actual proof payload,
        // so a message cannot be replayed with a swapped proof.
        if submission.message.payload_hash != submission.expected_payload_hash() {
            return Err(ProofError::PayloadHashMismatch.to_string());
        }
        ...
        // 2. Charge the anti-spam fee up front (refunded on success below).
        let fee = self.state.registry.params().proof_submission_fee;
        let charged = if fee > 0 {
            let balance = self.state.get_balance(&submitter);
            if balance < fee {
                return Err(ProofError::InsufficientFee {
                    have: balance,
                    need: fee,
                }
                .to_string());
        ...
        // Helper to burn (keep fee) — nothing to do, already deducted.
        // Helper to refund on any early return after verification success.

        // 3. Core-native STARK verification of untrusted input.
        let verify_result = <bud_proof::DefaultAdapter as bud_proof::ProverAdapter>::verify(
            &submission.proof,
            &submission.public_inputs,
            &submission.program,
        );
        if let Err(e) = verify_result {
            // Invalid proof: fee is burned (already deducted). State otherwise
            // unchanged.
            return Err(ProofError::InvalidProof(format!("{e:?}")).to_string());
        }

        // Proof is valid from here on → the fee will be refunded on every
        // non-error return path.
        let refund = |bc: &mut Self| {
            if charged {
                bc.state.add_balance(&submitter, fee);
            }
        };

        // 4. Apply the "first valid wins" claim policy.
        match self.proof_claims.classify(key, final_state_root) {
            Ok(ClaimDecision::Duplicate) => {
                // Idempotent re-submission of an already-accepted result.
                refund(self);
                Ok(ProofAcceptance::Idempotent)
            }
            Ok(ClaimDecision::New) => {
                // Persist the transport message (shared primitive) and record
                // the claim.
                self.submit_cross_domain_message(submission.message.clone())?;

                // 5. Reward only registered active provers.
                let rewarded = self.state.registry.is_active_prover(&submitter);
                let reward = self.state.registry.params().prover_reward;
                if rewarded && reward > 0 {
                    self.state.add_balance(&submitter, reward);
                }

                self.proof_claims.record(crate::prover::AcceptedProofClaim {
                    key,
                    final_state_root,
                    prover: submitter,
                    rewarded,
                ...
                refund(self);
                Ok(ProofAcceptance::Accepted {
                    rewarded,
                    reward: if rewarded { reward } else { 0 },
                })
            }
            Err(e) => {
                // Conflicting claim: valid proof but contradicts an accepted
                // result. Refund the fee (the prover was honest; the conflict is
                // a protocol-level rejection, not spam) and surface the error.
                refund(self);
                Err(e.to_string())
            }
        }
    ...
    /// Permissionless-but-staked relayer entry point for cross-domain messages.
    ///
    /// Enforces that `message.sender` is an active relayer in the permissionless
    /// registry (active or unbonding, not slashed/unregistered) before inserting
    /// the message. There is no whitelist — the only requirement is a bonded
    /// RELAYER registration, consistent with the network's stake-based security.
    ///
    /// On failure the state is left unchanged and a descriptive error returned.
    ...
    pub fn produce_block(&mut self, producer_address: Address) -> Option<Block> {
        ...
        // Tur 10: observe-mode liveness when this block closes an epoch.
        self.maybe_observe_liveness_on_epoch_close(committed_index);
    ...
    pub fn get_state_snapshot(&self, height: u64) -> Option<crate::chain::snapshot::StateSnapshot> {
        ...
        // Tur 11: use the fallible path so a serialization failure aborts
        // snapshot production (returns None) instead of silently emitting an
        // empty/corrupt snapshot.
        let v2_bytes = match v2.try_to_bytes() {
            Ok(bytes) => bytes,
            Err(e) => {
                tracing::error!("Aborting snapshot at height {}: {}", height, e);
                return None;
            }
        };
    ...
    pub fn handle_prevote(&mut self, vote: Prevote) -> Result<(), String> {
        // Capture identity BEFORE the vote is moved into the aggregator, so an
        // ingest-time invalid-signature rejection can be attributed (Tur 15).
        let voter = vote.voter_id;
        let aggregator = self
            .finality_aggregator
            .as_mut()
            .ok_or("No active finality aggregator")?;
        // Ingest the vote; equivocation evidence may be recorded even when the
        // vote itself is rejected (e.g. conflicting-hash), so drain it BEFORE
        // propagating the ingest result.
        let ingest = aggregator.add_prevote(vote).map_err(|e| e.to_string());
        self.route_detected_equivocations();
        self.maybe_track_invalid_vote(&ingest, voter);
        ingest
    }

    /// If an ingest result is an invalid-signature rejection, record it against
    /// the sender's per-epoch invalid-vote counter and, if the counter crosses
    /// `RegistryParams::max_invalid_votes_per_epoch`, route the resulting
    /// `InvalidSignatureSpam` report through the SINGLE canonical slash path
    /// (Tur 15, Görev 2). No second slashing path is opened.
    ///
    /// Only invalid-*signature* errors count: a signature error can only be
    /// returned AFTER the voter passed the validator-set membership check in the
    /// aggregator, so the sender is guaranteed to be a known validator.
    fn maybe_track_invalid_vote(&mut self, ingest: &Result<(), String>, voter: Address) {
        let is_invalid_sig = matches!(ingest, Err(e) if e.contains("signature"));
        if !is_invalid_sig {
            return;
        }
        let epoch = self.state.epoch_index;
        let params = *self.state.registry.params();
        if let Some(report) = self
            .state
            .invalid_votes
            .record_invalid_vote(epoch, voter, &params)
        {
            match self.submit_registry_slashing_report(report) {
                Ok(Some(_)) => info!("Invalid-signature spam slashed: validator {}", voter),
                Ok(None) => {}
                Err(e) => tracing::warn!("Invalid-vote-spam report rejected: {}", e),
            }
        }
    }

    /// Drain any equivocation (double-sign) evidence the active aggregator
    /// detected during vote ingest and route each report through the SINGLE
    /// canonical slashing path (`submit_registry_slashing_report`). This does
    /// NOT open a second slashing path — it reuses the exact flow that
    /// consensus double-sign and liveness reports already use.
    fn route_detected_equivocations(&mut self) {
        let reports = match self.finality_aggregator.as_mut() {
            Some(agg) => agg.take_detected_equivocations(),
            None => return,
        };
        for report in reports {
            match self.submit_registry_slashing_report(report) {
                Ok(Some(_)) => info!("Equivocation slashed via finality aggregator"),
                Ok(None) => {}
                Err(e) => tracing::warn!("Equivocation report rejected: {}", e),
            }
        }
    }
    ...
    pub fn handle_precommit(
        ...
        let aggregator = self
            ...
            .as_mut()
            .ok_or("No active finality aggregator")?;
        let ingest = aggregator.add_precommit(vote).map_err(|e| e.to_string());
        self.route_detected_equivocations();
        self.maybe_track_invalid_vote(&ingest, voter);
        ingest?;
