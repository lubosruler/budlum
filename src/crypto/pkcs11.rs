...
impl Pkcs11Signer {
    pub fn new(
        ...
        let pin = std::env::var(&token_pin_env)
            .map_err(|e| CryptoError::KeyGeneration(format!(
        ...
        let pkcs11_client = cryptoki::context::Pkcs11::new(&module_path)
            .map_err(|e| CryptoError::KeyGeneration(format!(
                "Failed to load PKCS#11 module '{}': {}",
                module_path, e
            )))?;

        pkcs11_client.initialize(cryptoki::context::CInitializeArgs::OsThreads)
            .map_err(|e| CryptoError::KeyGeneration(format!(
                "Failed to initialize PKCS#11: {}",
                e
            )))?;

        let slots = pkcs11_client.get_slots_with_token()
            .map_err(|e| CryptoError::KeyGeneration(format!(
                "Failed to enumerate PKCS#11 slots: {}",
                e
            )))?;

        let target_slot = slots.iter().find(|s: &&cryptoki::slot::Slot| s.id() == slot_id)
        ...
        let session = pkcs11_client.open_ro_session(*target_slot)
            .map_err(|e| CryptoError::KeyGeneration(format!(
        ...
        session.login(cryptoki::session::UserType::User, Some(&pin_secret))
            .map_err(|e| CryptoError::KeyGeneration(format!(
                "PKCS#11 login failed: {}",
                e
            )))?;

        let public_key_bytes = Self::extract_ed25519_public_key(&session)
            .map_err(|e| CryptoError::KeyGeneration(format!(
...
impl ConsensusSigner for Pkcs11Signer {
    ...
    fn sign_block(&self, block_hash: &[u8; 32]) -> Result<Vec<u8>, CryptoError> {
        let guard = self.inner.lock().map_err(|_| {
            CryptoError::Signing("PKCS#11 inner mutex poisoned".to_string())
        })?;
        let inner = guard.as_ref().ok_or_else(|| {
            CryptoError::Signing("PKCS#11 session already closed".to_string())
        })?;
        ...
        let objects = inner.session.find_objects(template)
            .map_err(|e| CryptoError::Signing(format!(
                "Failed to find Ed25519 private key: {}",
                e
            )))?;
        if objects.is_empty() {
        ...
        let signature = inner.session.sign(&mechanism, key_handle, block_hash)
            .map_err(|e| CryptoError::Signing(format!(
                "HSM sign operation failed: {}",
                e
            )))?;
