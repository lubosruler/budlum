# Mainnet Lockdown Checklist — Phase 11.20
...
**Scope:** final pre-launch review after Phase 11.6 → 11.20 evidence is present.
...
| Wallet core | `Wallet Core (Phase 11.14)` | Must be green |
| Governance | `Governance Invariants (Phase 11.16)` | Must be green |
| PoA compliance isolation | `PoA Compliance Isolation (Phase 11.18)` and legacy PoA isolation gate | Must be green |
| Audit/HSM docs | `Audit Prep (Phase 11.20)`, validator key-management, HSM policy | Must be green |
...
6. Confirm all Phase 11.8, 11.10, 11.12, 11.14, 11.16, 11.18 and 11.20 gates are green on main.
...
A waiver can never bypass CI. It can only postpone a non-launch-blocking operational task. Waivers must include:
