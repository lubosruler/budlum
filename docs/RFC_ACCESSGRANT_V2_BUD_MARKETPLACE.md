...
| **İlgili** | GAP-1 RFC (APPROVED — tek schema-4 birleşimi) ·  Bölüm 1/2/3 · B.U.D. E2E invariantları |
...
- R3 (kritik sınıf): `BTreeMap<[u8; 32], _>` serde_json'da map-key olarak **çalışmaz** (JSON object-key = string; ekip bunu `permissionless.rs:176` tuple-key notuyla ve  registry bug'ıyla yaşadı). v2'de **AssetId wrapper** (§3.1) — Address deseniyle string-serialize (`src/core/address.rs:64-73`).
...
**Mimari kısıt (veri egemenliği):** whitelist/admin/pause/freeze hook'u YASAK; her RPC her node'dan sunulabilmeli → ** hard-enforcement (şifreleme) mimari kısıt gereği zorunludur**;  soft-enforcement ancak "imza + zincir üstü kayıt" ile anlamlıdır (v1 bunu TODO bırakmıştı).
...
**Boundedness kuralı (üç-kapı #3):** imza/key alanları sabit-boyut (`Signature64`, `[u8; 32]`); `Vec<u8>` yalnız -2 `wrapped_key`'de, üst sınır `MAX_WRAPPED_KEY = 1024` byte (decode'da enforce).
...
- ** (bu RFC'nin implementasyonu):** §3-§7 — imza-doğrulamalı soft-enforcement + consumption registry + root bağlantılı registry'ler.
- ** (hard-enforcement, ARENA3 HSM domain'i):** `encrypted=true`, AES-256-GCM DEK + **HPKE (RFC 9180)** wrapped_key (v1 §10 kararı: HPKE=BEKLEYEN — bu RFC'de karar: HPKE, HSM uyumlu); storage node plaintext'i asla görmez; `MAX_WRAPPED_KEY` enforcement.
...
1. `Perpetual` scope 'de key-rotasyonla sınırlanacak — 'de de `valid_until` üst sınırı zorunlu mu? (öneri: hayır; revocation mekanizması yeterli)
2. `once_consumed` büyümesi prunig'siz kalıcı mı? (öneri: kalıcı; provenance zinciri)
3. 'de `encrypted=false` iken grant'ın teknik caydırıcılığı sınırlı (ham veri açık) —  zaman çizelgesi kullanıcıyla.
