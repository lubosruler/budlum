...
//  0.31 (security audit Z-A): public-input binding witness columns.
//
...
// D2 (2026-07-22)  D: privacy-layer opcode selectors.
...
//  0.312 (security audit Z-B): Merkle path verification columns.
//
...
//  0.312 (security audit Z-B), Commit 3: final root check
// witnesses.
...
impl<AB: PermutationAirBuilder> Air<AB> for BudAir {
    fn eval(&self, builder: &mut AB) {
        ...
        //  0.31 (security audit Z-B): the `is_verify_merkle`
        ...
        // `TASK0.30.5-PLAN.md` and will land in  0.312.
        ...
        //  0.312 (security audit Z-B): Merkle expansion rows.
        //
        ...
        //  0.36 / Z-B 3.5: merkle_round is 0..63 (not boolean). Bound via
        ...
        // the trace).
        //  0.36 / Z-B 3.5: key continuity only when staying in / entering
        ...
        //  0.312 (security audit Z-B), Commit 3: Poseidon
        ...
        //  0.312 Commit 3, final root check: on the original
        ...
        // merkle_final_flag = 1).
        //  0.36 / Z-B 3.5: equality boolean via inverse witness on the
        ...
        //  0.312 Commit 3 + ARENA2  4 fix: leaf binding and first-round
        ...
        //  0.30 (security audit Z-C): termination constraint.
        //
        ...
        //  0.338 / A12: SRead=8, SWrite=12 (must match Vm::gas_cost).
        let gas_cost = is_load.clone() * three.clone()
            ...
            //  0.36: expansion rows reuse opcode 0x1E but must not re-charge gas.
        ...
        //  0.31 (security audit Z-A): bind the remaining public
        ...
        //  0.31 (security audit Z-A,  2): event_digest,
        ...
        // (5b) event_digest transition ( 0.358 fix):
        ...
        if rand.len() >= 3 && perm_cur.len() >= 3 && perm_nxt.len() >= 3 {
            ...
            //  8.10 (ARENA3 aux CTL fix #2): Register LogUp must also
            ...
            let cpu_active: AB::Expr = cur[COL_CPU_ACTIVE].into();
            //  8.10 (ARENA3 aux CTL fix): VerifyMerkle expansion rows
