...
> `budlum-core` (ağ / L1) ve Phase 0.37'te aynı repoya alınan `budzero/`
...
- RPC API: Relayer modeli netleştiği için (permissionless) RPC uç noktaları
  ...
  sorgulama uçları dahil. **Phase 0.38, Faz 5 — B.U.D. storage RPC yüzeyi
  (permissionless)**: `bud_storageRegisterManifest`,
  `bud_storageGetManifest`, `bud_storageGetDealsByManifest`,
  `bud_storageGetDealsByShard`, `bud_storageOpenChallenge`,
  `bud_storageAnswerChallenge`, `bud_storageGetOutcome`. Hepsi
  herkes tarafından çağrılabilir; admin-only uç YOK; whitelist YOK;
  "Budlum Inc. indexer" servisi bağımlılığı YOK (data-sovereignty
  kuralı, Phase 0.39 plan §0.5).
...
- **Monorepo (Phase 0.37):** BudZero/BudZKVM kaynakları `budzero/` altındadır.
...
- **B.U.D. (Phase 0.38):** `ConsensusKind::StorageAttestation(StorageDomainParams)`
  yeni enum varyantı (vizyon §8.1, Custom(String) DEĞİL). Muhasebe Faz 1 =
  yalnızca domain kaydı; Faz 2 = content addressing + manifest;
  Faz 5 = deal/challenge ekonomisi. **`RetrievalChallenge` interim'dir** —
  Faz 3 (gerçek Proof-of-Storage) BudZKVM `VerifyMerkle` 64-depth
  production gate'ine bağlıdır. Z-B gate kapanana kadar "proof-of-storage"
  iddiası YAPILMAZ (sahte-yeşil yol riski, vision §9.1).
...
- `src/registry/role.rs` — generic `RoleId` (hard-coded enum değil). Bilinen
  roller: `roles::VALIDATOR / VERIFIER / RELAYER / PROVER / STORAGE_OPERATOR`.
  `STORAGE_OPERATOR` (RoleId 5) **Phase 0.38, Faz 1**: permissionless, stake ile
  kayıt; whitelist YOK. (Deal açmak için bu role gerekli DEĞİL — `open_deal`
  permissionless; `STORAGE_OPERATOR` yalnızca ödül/eligibility için
  kullanılacak, Faz 5 takibi.)
...
- `src/domain/storage_params.rs` — **Phase 0.38, Faz 1 (B.U.D.)**:
  `StorageDomainParams` (chunk_size, max_committed_chunks,
  challenge_interval, min_operator_bond) + `validate()` + domain-tagged
  `storage_params_bytes()`. Bkz. vizyon §8.1.
- `src/domain/storage_deal.rs` — **Phase 0.38, Faz 5 (B.U.D. deal/challenge
  ekonomisi)**: `StorageDeal` + `StorageRegistry` (BTreeMap-backed,
  permissionless) + `RetrievalChallenge`/`Response`/`Outcome`/
  `ChallengeResult`. KRİTİK SINIRLAMA: `RetrievalChallenge` **gerçek
  Proof-of-Storage DEĞİLDİR** — operatör sadece istenen byte-range'i
  saklayarak testi geçebilir. Tam kanıt (Faz 3, vision §8.3) BudZKVM
  `VerifyMerkle` + 64-depth SMT production gate'ine bağlıdır; o
  Z-B gate'i kapanana kadar "interim retrieval challenge" etiketi
  zorunludur (README/CLAUDE.md'de açıkça yazılı). Whitelist/
  admin/pause hook'u YOK — data-sovereignty kuralı (Phase 0.39 plan §0.5).
- `src/storage/content_id.rs` — **Phase 0.38, Faz 2 (B.U.D. content
  addressing)**: `ContentId` (32-byte domain-tagged SHA-256).
  `of_subrange` byte-range hash (interim) — gerçek proof DEĞİL.
- `src/storage/manifest.rs` — **Phase 0.38, Faz 2**: `ContentManifest` +
  `ShardRef`. Parçalama off-chain; zincir sadece `(manifest_id, shard_id,
  size)` taşır (BudZKVM STARK kanıtı analoğu).
- `src/domain/finality_adapter.rs` — domain finality doğrulayıcıları. Güvenlik
  durumu (Phase 0.08-7 sonrası — TÜM aile gerçek doğrulama yapıyor): PoS gerçek BLS
  cert doğrular (`cert.verify`); **BFT** aynı desende gerçek BLS cert doğrular
  (self-reported signer_count KALDIRILDI, Phase 0.10); **PoW** proof'u commitment'a
  bağlar (`declared_head_hash == commitment.domain_block_hash`) +
  declared_cumulative_work iç tutarlılık + min-work eşiği (Phase 0.10; tam
  light-client DEĞİL, o ayrı iş); ZK `ProofClaimRegistry` üzerinden gerçek STARK
  (Phase 0.08); **PoA** gerçek ed25519 imza kümesi doğrular (`FinalityProof::PoA` artık
  `authorities: Vec<Address>` + `signatures: Vec<PoAAuthoritySignature>` taşır,
  `poa_commit_signing_message` ile commitment'a bağlı, SAYI-bazlı quorum — Phase 0.12).
  KRİTİK: PoA doğrulaması stake mekanizmasına/permissionless registry'ye
  BAĞLANMADI; PoA kendi ayrı stake-siz ed25519 modelini kullanır (Phase 0-2
  izolasyonu korunur). PoA'da stake YOK (bilinçli). Yeni proof alanı eklerken
  `#[serde(default)]` kullan (geriye uyumluluk).
- `src/tokenomics/mod.rs` — $BUD tokenomik (Phase 0.14). Arz 100M, **6 ondalık**
  ...
  ENTEGRASYON (Phase 0.14b — gerçek zincir akışına bağlı): (a) genesis:
  ...
  `team_vesting` DÜZELTİLDİ (Phase 0.16): artık snapshot'a girip çıkıyor.

- State persistence (Phase 0.16): `StateSnapshotV2` `schema_version=3` ile `registry`,
...
- Sessiz-hata sertleştirmesi (Phase 0.32): serialize-into-hash/persistence/network
...
- Liveness → epoch akışı (Phase 0.30): `Blockchain::maybe_observe_liveness_on_epoch_close`
...
- Liveness DISPATCH (Phase 0.34): `maybe_observe_liveness_on_epoch_close` artık
  ...
  * `false` (VARSAYILAN) → Phase 0.30 `observe_liveness_epoch` = rapor + `tracing`
  ...
  `true` yapınca aktifleşir (Phase 0.30 "önce gözlemle, canlı/testnet'te doğrula,
  sonra aç" kuralı). `record_liveness_epoch` mekanizması Phase 0.04'ten beri var+test
  edilmişti; Phase 0.34 sadece canlı akıştan erişilebilir kıldı. Kademeli ceza
  (liveness'te jail yok) ayrı/gelecek tur — `slash()`'a dokunulmadı.
- Adversarial finality testleri (Phase 0.36-14): `src/tests/finality_adversarial.rs`.
...
- Aggregator sağlamlaştırması (Phase 0.38, `FinalityAggregator`):
...
- Slashing kalıcılığı + geçmiş (Phase 0.40, Görev 1): `PermissionlessRegistry`'ye
...
- Tekrarlı geçersiz-imza slashing (Phase 0.40, Görev 2): yeni `InvalidVoteTracker`
...
- Prover → L1 köprüsü: `Blockchain::submit_zk_proof(ZkProofSubmission)`. Model B
  ...
  ödül; gönderim kapısı DEĞİL — Phase 0.04 relayer kapısıyla karıştırma).
